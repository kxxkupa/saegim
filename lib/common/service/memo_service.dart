...
class MemoService {
  ...
  Stream<List<MemoData>> watchAllMemos() {
    return (db.select(db.memo)..orderBy([(tbl) => OrderingTerm.desc(tbl.date), (tbl) => OrderingTerm.desc(tbl.id)])).watch();
  }

  // watchAllMemos의 데이터를 받아서 그룹화하여 반환하는 Stream
  Stream<Map<String, List<MemoData>>> watchGroupedMemos() {
    return watchAllMemos().map((allMemos) {
      final Map<String, List<MemoData>> groupedMemos = {};
  ...
  Future<void> saveMemo({
    ...
    if (id != null) {
      ...
      final memoCompanion = MemoCompanion(
        title: Value(title),
        content: Value(content),
      );

      await (db.update(db.memo)..where((tbl) => tbl.id.equals(id))).write(memoCompanion);
    } else {
      ...
      await db.into(db.memo).insert(memoCompanion);
    }
  }

  // 메모 삭제
  Future<void> removeMemo(int id) async {
    await (db.delete(db.memo)..where((tbl) => tbl.id.equals(id))).go();
  }
}