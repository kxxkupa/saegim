...
class MemoDao extends DatabaseAccessor<LocalDatabase> with _$MemoDaoMixin {
  ...
  Stream<List<MemoData>> watchMemos() {
    return (select(memo)..orderBy([(tbl) => OrderingTerm.desc(tbl.date), (tbl) => OrderingTerm.desc(tbl.id)])).watch();
  }
  ...
  // 메모 삭제
  Future<int> removeMemo(int id) => (delete(memo)..where((tbl) => tbl.id.equals(id))).go();
}